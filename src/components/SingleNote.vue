    ...
    <div class="px-4">
        <section class="flex -mx-4 flex-wrap">
            <div :key="oneNote.i" class="md:w-1/4 sm:w-1/2 shadow-lg px-4 py-6"
                ...
                <span @click="finishTask(i)" class="absolute pin-r cursor-pointer"><svg class="feather feather-square" fill="none"
                    ...
                    <input autofocus="true"
                           ...
                           placeholder="Type a title ..." type="text"
                           v-if="edit && editId == i"
                           v-model="oneNote.title" style="max-width: 92%;">
                    <h2 :class="{isDone: oneNote.completed}" v-else class="text-left overflow-auto" style="max-width: 92%;">{{oneNote.title}}</h2>
                    <small class="mt-4">Last update: {{oneNote.date}}</small>
                    <hr>
                    <textarea class="w-full shadow-inner p-4 border-0 rounded bg-transparent"
                              placeholder="Type a description ..."
                              v-if="edit && editId == i"
                              v-model="oneNote.text"></textarea>
                    <p :class="{expand: idToExpand == i, isDone: oneNote.completed}" v-else
                       v-html="modifiedText(i)" class="text-left overflow-x-auto h-16 break-words"></p>
                    <div class="flex justify-between" :class="(idToExpand == i) ? 'mt-2' : 'mt-10' ">
                    <span @click="updateNote(i, oneNote)" v-if="edit && editId == i" class="cursor-pointer"><svg class="feather feather-check"
                                                                                          fill="none" height="24"
                                                                                          stroke="currentColor"
                                                                                          stroke-linecap="round"
                        ...
                        <span @click=editNote(i) v-else class="cursor-pointer"><svg class="feather feather-edit" fill="none" height="24"
                        ...
                        <span @click="toggleTransition(i)" class="cursor-pointer"><svg class="feather feather-folder-plus" fill="none"
                                ...
                                x1="12" x2="12" y1="11" y2="17"></line><line x1="9" x2="15" y1="14"
                                                                             y2="14"></line></svg></span>
                        <span @click="deleteNote(i)" class="cursor-pointer"><svg class="feather feather-trash-2" fill="none" height="24"
                                                          stroke="currentColor" stroke-linecap="round"
                                                          stroke-linejoin="round"
                                                          stroke-width="2" viewBox="0 0 24 24"
                                ...
                                x1="10" x2="10" y1="11" y2="17"></line><line x1="14" x2="14" y1="11"
                                                                             y2="17"></line></svg></span>
                        <span @click="expandNote(i)" v-if="oneNote.long" class="cursor-pointer"><svg class="feather feather-maximize"
                                                                              fill="none" height="24"
                                                                              stroke="currentColor"
                                                                              stroke-linecap="round"
                                                                              stroke-linejoin="round"
